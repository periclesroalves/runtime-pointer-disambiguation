...
void f1B(int i, uint32 *nGroupbys_, TUPLE_ONES *toptr_, uint64 *gbuf_,
        ADC_VIEW_CNTL *avp) {
  int j;

  for(j = 0; j < nGroupbys_[i]; j++ ){
    toptr_[j].tuple = gbuf_[j];
    toptr_[j].nOnes  = countTupleOnes(gbuf_[j], avp->nTopDims);
  }
  qsort((void*)gbuf_,  nGroupbys_[i], 8, Comp8gbuf );
  onessort(toptr_, nGroupbys_[i]);

  for(j = 0; j < nGroupbys_[i]; j++){
    toptr_[nGroupbys_[i]-1-j].tuple <<= (64-avp->nTopDims);
    swap8(&toptr_[nGroupbys_[i]-1-j].tuple);
    fwrite(&toptr_[nGroupbys_[i]-1-j].tuple, 8, 1, avp->groupbyFile);
  }
}
...
uint32 MultiFileProcJobs( TUPLE_VIEWSIZE *tuplesAndSizes, 
   ...
   for(i = 0; i < avp->nTasks; i++ ){
     if(i == avp->taskNumber){
       f1B(i, nGroupbys, toptr, gbuf, avp);
     }
   }

...
void f1A(ADC_VIEW_CNTL *adccntl_) {
  int i;

  for ( i = 0; i < adccntl_->nm; i++ ){
    adccntl_->mSums[i] = 0;
    adccntl_->checksums[i] = 0;
    adccntl_->totchs[i] = 0;
  }
}

void f2A(ADC_VIEW_CNTL *adccntl_) {
  int i;

  for ( i = 0; i < adccntl_->nv; i++ ) adccntl_->selection[i]=i+1;
}

...
#pragma omp parallel for private(lbv,ubv)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(n2-2,28);t2++) {
      for (t3=0;t3<=floord(n1-1,256);t3++) {
        for (t5=max(1,28*t2);t5<=min(n2-2,28*t2+27);t5++) {
          lbv=max(2,256*t3);
          ubv=min(n1-1,256*t3+255);
#pragma ivdep
...
#pragma omp parallel for private(lbv,ubv)
  for (t1=lbp;t1<=ubp;t1++) {
    for (t2=0;t2<=floord(n2-2,20);t2++) {
      for (t3=0;t3<=floord(n1-1,256);t3++) {
        for (t5=max(1,20*t2);t5<=min(n2-2,20*t2+19);t5++) {
          lbv=max(2,256*t3);
          ubv=min(n1-1,256*t3+255);
#pragma ivdep
#pragma vector always

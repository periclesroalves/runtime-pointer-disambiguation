...
void f1(int nelt_, int *front_, int *newc_, int *count_, int vassign_[][8]) {
  int iel;

  for (iel = 0; iel < nelt_; iel++) {
    // compute the starting vertex mortar point index in element iel
    front_[iel] = front_[iel]-newc_[iel];

    for (i = 0; i < newc_[iel]; i++) {
      ...
      // this vertex point.
      *count_ = front_[iel]+i;
      mortar_vertex(vassign_[iel][i], iel, *count_);
    }
  }
}
...
void mortar()
{
  ...
  f1(nelt, front, newc, count, vassign);
...
static void f2(int ie, int n, int ng, int *nemo_)
{
  int ntemp, i;
  logical L1;

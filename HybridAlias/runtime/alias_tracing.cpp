...
typedef map< void *, void *, greater<void *>, BootstrapAllocator< pair<void *, void *> > > Map;

static inline Map& lut();
static FILE *init_tracer();

struct InitTracer {
	InitTracer() : file(init_tracer()) {}

	FILE *const file;
};

static InitTracer tracer;
...
/// Look-up table for memory addresses.
static inline Map& lut() {
	static Map *_lut;
...
static void * __attribute__((deprecated)) my_malloc_hook(size_t size, const void *caller)
{
	...
	lut()[ptr] = reinterpret_cast<void *>(reinterpret_cast<char *>(ptr) + size-1);

/*
  ...
  Simple executable that wraps the ILC pass
*/
...
cl::opt<string> TraceFilePositional(cl::Positional, cl::desc("<ALIAS YAML>"), cl::Optional);
cl::opt<string> CloneInfoFilePositional(cl::Positional, cl::desc("<CLONE INFO>"), cl::Optional);
...
int main(int argc, char **argv) {
	...
	cl::ParseCommandLineOptions(argc, argv, "Inserts alias profiling code");
	...
	// set up some stuff for ILC

	if (TraceFile.empty() && TraceFilePositional.empty())
	{
		ERR("No alias data file given");
		return 1;
	}

	if (TraceFilePositional.size())
	{
		TraceFile.setValue(TraceFilePositional);
	}

	if (CloneInfoFilePositional.size())
	{
		CloneInfoFile.setValue(CloneInfoFilePositional);
	}

	// clear file containing cloned loops

	if(CloneInfoFile.size()) {
		std::error_code err;

		raw_fd_ostream file{CloneInfoFile.getValue(), err, sys::fs::F_Text};

...
Value *AliasInstrumenter::getBasePtrValue(Instruction &inst, Region *r) {
  Value *ptr = getPointerOperand(inst);
  Loop *l = li->getLoopFor(inst.getParent());
  const SCEV *accessFunction = se->getSCEVAtScope(ptr, l);
  const SCEVUnknown *basePointer =
...
bool AliasInstrumenter::tryInstrumentDependencies(Region *r) {
  ...
  for (BasicBlock *bb : r->blocks()) {
    for (BasicBlock::iterator i = bb->begin(), e = --bb->end(); i != e; ++i) {
      ...
      // If we can't define the base pointer, then it's not safe to instrument.
      if (!basePtrValue)
        return false;
      ...
      Loop *l = li->getLoopFor(inst.getParent());
      const SCEV *accessFunction = se->getSCEVAtScope(ptr, l);
      memAccesses[basePtrValue].insert(accessFunction);
      ...
      if (!as.isMustAlias()) {
        for (const auto &aliasPointer : as) {
          ...
          if (basePtrValue <= aliasValue)
            pairsToCheck.insert(std::make_pair(basePtrValue, aliasValue));
          else
            pairsToCheck.insert(std::make_pair(aliasValue, basePtrValue));
        }
      }
    }
  }
...
bool AliasInstrumenter::isValidInstruction(Instruction &inst) {
...
bool AliasInstrumenter::isSafeToInstrument(Region &r) {
...
void AliasInstrumenter::findAndInstrumentRegions(Region &r) {
  // If the whole region was successfully instrumented, stop the search.
  if (isSafeToInstrument(r) && tryInstrumentDependencies(&r))
    return;
...
bool AliasInstrumenter::runOnFunction(llvm::Function &F) {
  ...
  declareTraceFunction(F.getParent());
  currFn = &F;
...
INITIALIZE_PASS_BEGIN(AliasInstrumenter, "polly-scev-checks",
...
INITIALIZE_PASS_END(AliasInstrumenter, "polly-scev-checks",

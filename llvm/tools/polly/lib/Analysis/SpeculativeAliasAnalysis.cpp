...
namespace {
  struct NoSpecAA : ImmutablePass, SpeculativeAliasAnalysis {
    static char ID;
    NoSpecAA() : ImmutablePass(ID) {}

    SpeculativeAliasResult speculativeAlias(const Value *, const Value *) override {
...
INITIALIZE_AG_PASS(NoSpecAA, SpeculativeAliasAnalysis, "no-spec-aa",
                   "No Speculative Alias Analysis (always returns DontKnow)",
                   true, true, true)
